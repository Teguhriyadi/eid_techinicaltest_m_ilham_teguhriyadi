...
@push('content-modules')
    ...
    <div class="card shadow mb-4">
        ...
        <form action="{{ url('/pages/produksi/' . $produksi['id']) }}" method="POST">
            @csrf
            @method("PUT")
            <div class="card-body">
                <div class="row">
                            ...
                            <select name="mesin_id" class="form-control @error('mesin_id') is-invalid @enderror">
                                ...
                                @foreach ($mesin as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('mesin_id', $produksi['mesin_id']) == $item->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select name="operator_id" class="form-control @error('operator_id') is-invalid @enderror">
                                ...
                                @foreach ($operator as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('operator_id', $produksi['operator_id']) == $item->id ? 'selected' : '' }}>
                                        {{ $item->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="shift" class="form-control @error('shift') is-invalid @enderror">
                                ...
                                @foreach ($shift as $item)
                                    <option value="{{ $item }}"
                                        {{ old('shift', $produksi['shift']) == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number"
                                ...
                                step="0.01"
                                name="temperatur"
                                value="{{ old('temperatur', $produksi['temperatur']) }}"
                        ...
                        <div class="form-group">
                            ...
                            <select name="status"
                                ...
                                @foreach ($status as $item)
                                    <option value="{{ $item }}"
                                        {{ old('status', $produksi['status']) == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
                            </select>
