...
@push('content-modules')
    ...
    <div class="card shadow mb-4">
        ...
        <form action="{{ url('/pages/mesin/' . $mesin['id']) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" name="kode_mesin" class="form-control @error('kode_mesin') is-invalid @enderror"
                        value="{{ old('kode_mesin', $mesin['kode_mesin']) }}" placeholder="Contoh : CNC-001">
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control @error('status') is-invalid @enderror">
                        ...
                        @foreach ($status as $item)
                            <option value="{{ $item }}"
                                {{ old('status', $mesin['status']) == $item ? 'selected' : '' }}>
                                {{ $item }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label>Temperatur (°C) <span class="text-danger">*</span></label>
                    <input type="number" min="0" max="999.99" step="0.01" name="temperatur"
                        class="form-control @error('temperatur') is-invalid @enderror"
                        value="{{ old('temperatur', $mesin['temperatur']) }}" placeholder="Contoh : 42.50">
                    @error('temperatur')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
